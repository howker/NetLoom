...
- До отдельной переработки оболочки новые постоянные feature/editor surfaces реализуются как панели или режимы внутри `MainWindow`, а не как новые самостоятельные WPF `Window`; системные file/folder pickers и короткие атомарные dialogs допустимы, когда операция действительно модальна.
- Новые постоянные элементы управления не добавляются в существующую верхнюю toolbar `MainWindow`; используется существующая панель/раздел либо управление откладывается до shell work, если для него нужна новая навигационная концепция.
...
- Автоматизированный тест не доказывает product behavior чтением C#/XAML/SQL/PowerShell как plain text и поиском имён методов, event handlers, вызовов, фрагментов реализации или случайных констант. Такой source-text assertion запрещён как подмена поведенческой проверки.
- Поведенческий тест строится как `input → действие через production boundary → наблюдаемый результат`. Для persistence используется реальный round-trip `save → recreate/read → compare`; для interaction — реальный event/command path, если он надёжно автоматизируем.
- Если UI-поведение нельзя надёжно проверить без запуска живого WPF interaction, оно фиксируется как пункт operator acceptance, а не имитируется поиском строк в исходниках.
- Static/integrity check допустим только когда сам artifact является предметом проверки (например, UTF-8, resource-key parity, structured schema/resource integrity, license/dependency boundary). Такой check не считается behavioral coverage, должен быть явно назван integrity/static-analysis проверкой и по возможности разбирать структурированный формат вместо поиска подстроки. Для нового blocking integrity check сохраняется требование доказать RED на известном плохом fixture/контролируемой регрессии.
- Рефакторинг, сохраняющий наблюдаемое поведение, не должен требовать изменения behavioral tests только из-за переименования/переноса файла, метода, partial-класса или handler-а. Количество тестов само по себе не является acceptance metric.
...
- Один принятый operator outcome имеет один Sprint number от начала до финальной operator acceptance. Внутренние slices/commits не получают суффиксы `A/B/C`; пока общий outcome не принят, работа остаётся тем же Sprint. Исторические labels не переписываются задним числом, но новые документы не должны продолжать буквенное дробление.
