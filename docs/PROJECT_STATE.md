...
Sprint 39 — visual language of the map — is technically complete and operator-accepted. The accepted product implementation is represented by incremental commits `4c9104ce2879a3159802e16b93951a516360c55b`, `7066ca0c4fb07d660448eb0a3a898d25e8135879`, `5cdb5423b4083eb9dfab8cf899b24f4313d077e7`, and the holistic operator-remediation commit `551472caf2e767c92025fa266d03f356807e697b`.

The map now uses independent visual channels for link confidence and freshness: `High` / `Medium` / `Low` control solid / long-dash / short-dash line grammar, while `Fresh` / `Aging` / `Stale` control opacity. Selection remains a separate interaction layer and does not erase the underlying confidence/freshness representation.

Existing STP/ring/forwarding-safety evidence is consumed by the renderer rather than inferred from geometry. Confirmed forwarding-cycle or `Broken` evidence is Critical; degraded ring protection or `Disabled` is Degraded; `Blocking` is Blocked; `Listening` / `Learning` is Transition; confirmed `Forwarding` is visually healthy; unresolved/unknown evidence stays neutral. Node degradation is truthful and independent of link state: only existing `DeviceDiagnostic` evidence with at least one `Degraded` interface produces degraded-node treatment.

Holistic operator acceptance found that the first technical styling was not differentiated enough and lacked a fast way to isolate problem states. The accepted remediation adds compact state-dependent stroke widths, stronger semantic colors and `Map settings → Focus by state` for all problems, Critical, Degraded, Blocked, Transition and degraded nodes. Clearing focus removes only the focus/dimming layer: it preserves pan/zoom and the current selection. Wheel zoom with a selected node, link or Location keeps that selected element centered; with no selection, cursor-centric zoom remains unchanged.

A blocking post-Sprint-39 test-semantics cleanup was then completed without product-behavior changes. Source-text pseudo-contract tests that searched C#/XAML/SQL for method names, handlers, implementation fragments or incidental constants were removed or replaced with behavioral evidence where a production boundary exists. Persistence is now proved through a real store round-trip; UI behavior that cannot be reliably automated remains operator-acceptance evidence. `AI_DEVELOPMENT_RULES.md` now requires `input → production action → observable result`, separates integrity/static-analysis checks from behavioral coverage, and states that one operator outcome keeps one Sprint number instead of `A/B/C` slicing. Cleanup commit `7b78b1818dca3a0048f4d3d93a77255ed8309866` passed full Unit, Integration, Modern and Snapshot regression, repository text-integrity, exact repository-boundary/blob/deletion proofs and push; `HEAD == origin/main` and the worktree were clean.

Sprint 36 calm semantic motion remains the accepted animation baseline: `Normal` / `Reduced` / `Off` applies to semantic appearance/disappearance, freshness changes, search focus and one new-alert pulse, with no perpetual decorative motion. Sprint 39 did not add a second animation system. One-shot operational/degradation state-transition motion is recorded as a candidate, not a committed reorder.

Sprint 39 adds no database migration; `Migration020InterfaceIdentityAndManagementAddress` remains the current schema migration. No Sprint 39 product change remains open.
...
The operator target always includes the selected diagnostic device's stable `DeviceId`. The monitoring panel also owns an editable target-address field. Selecting a device pre-fills that field from observed `management_address` when available; when no address has yet been observed, the operator may enter a valid IP address manually so the first poll is possible. A manual target address is session launch input only and is not persisted back into `devices.management_address`, because that column remains observed management metadata rather than operator configuration. A device must still exist and be selected, so a fresh installation creates/selects a manual device before entering the first polling address. A running schedule keeps its active target; `Poll now` while running acts on that target and does not silently switch because selection changed. `Poll now` while stopped may execute one canonical Engine `poll-once` for the selected valid target/address.

The session policy surface mirrors the Engine knobs that already exist: schedule interval, SNMP version, UDP port, timeout, retry count, max repetitions, enabled poll kinds, and optional interface error/discard-rate-per-minute thresholds. Policy values and a manually entered target-address override are intentionally session-only in Sprint 40 and reset when Desktop restarts; persistence is recorded as a later candidate rather than hidden as an accidental omission. Sprint 40 intentionally keeps the existing `NETLOOM_SNMP_*` environment credential contract. It does not add secret editing, profile-to-target resolution, a new configuration database, persisted monitoring preferences, or production secret-management semantics.

`Refresh topology` remains the existing WPF coherent read refresh (`TopologyRefreshCoordinator`) rather than becoming another Engine command. UI status will expose monitoring state (`Stopped`, `Starting`, `Running`, `Polling`, `Stopping`, `Faulted`), active target, last successful Engine poll time, and the already tracked last successful topology-refresh time. The Desktop adapter owns only the child Engine process it launches. Redirected stdin remains the cooperative control/lifetime channel, while the Windows WPF host also assigns that child to a kill-on-close Job Object as a fail-safe for abrupt Desktop termination. Independently launched Engine processes remain outside Desktop ownership and Sprint 40 does not add global process arbitration; a startup/runtime failure or database/resource contention observed in the owned child becomes Faulted state with operator-visible diagnostics. Machine-readable Engine control/summary markers are ASCII-only, culture-invariant, newline-delimited and explicitly flushed; Desktop parses only those documented markers, not localized/free-form stdout/stderr. Topology truth continues to come from the database read model.

Verification follows the post-Sprint-39 test-semantics rule: behavioral tests for target-address fallback/override validation, policy validation, Engine command construction/parsing, stdin-control effects, child-process lifecycle/state transitions and marker parsing; existing scheduler/runtime tests remain authoritative for polling semantics. Live WPF click/selection behavior is operator acceptance, not source-text substring coverage. Sprint 40 adds no schema migration and does not include persisted monitoring preferences, global arbitration of independently launched Engine processes, multi-target scheduling, Service deployment, AccessProfile redesign, or the separate semantic-motion candidate.
