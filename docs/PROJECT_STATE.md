...
The realistic stand acceptance, Sprint 33A incremental WPF map reconciliation, Sprint 33B collision-safe link-label placement, Sprint 34A through Sprint 34K, the post-34K UI foundation task, Sprint 35 selected-element diagnostics, and Sprint 36 interactive persistent map are complete. The retained notification-delivery path remains supported but is not the active product priority.

Sprint 36 is operator-accepted. Initial implementation is `c9c6714b8663074be9864e8fb14f1c8355b76955`; the accepted navigation/virtual-workspace follow-up is `1ff950e45f5a4d67ae72dfce1dfdecc316d9999a`. WPF supports drag for unlocked nodes, visible/persistent node locking, middle-drag pan, `1%..500%` zoom, `Show all`, compact Help and a very large virtual workspace. Device positions plus viewport zoom/pan/lock state are persisted through `IMapLayoutStore` / `SqliteMapLayoutStore` and restored after Desktop restart. `Migration019MapLayout` is the current schema migration.
