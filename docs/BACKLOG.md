...
- [ ] Sprint 39 — visual language of the map.
  ...
  - Sprint 39A — link trust/freshness grammar on already available map data. `MapLink` already carries `MapConfidence` (`Low` / `Medium` / `High`) and `MapFreshness` (`Fresh` / `Aging` / `Stale`); encode confidence with line pattern and freshness with opacity while keeping the base link color semantically neutral. Selection remains an interaction layer: it owns selection color/thickness but must preserve operational dash/opacity. Do not add topology/domain contracts in 39A.
  - Sprint 39B — wire existing STP/ring/forwarding-safety classifications into the map presentation. Reuse `StpTreePortState`, `RingProtectionAnalysis` and `ForwardingCycleAnalysis`; do not infer blocked/risky state from geometry or confidence. Define explicit precedence for forwarding, blocking/disabled, unresolved, degraded/unprotected and confirmed-cycle presentation only after those existing classifications reach the renderer.
  - Sprint 39C — complete the remaining node/degradation visual language and run holistic operator acceptance. `MapNode` currently carries identity/origin/category/monitoring-capability but no confidence/freshness/degradation state, so node operational styling must be driven only by a truthful existing source wired into the WPF refresh path; do not synthesize node health from link state. Close Sprint 39 only after physical topology, active STP/ring/failure presentation, selection behavior and light/dark themes are coherent together.
