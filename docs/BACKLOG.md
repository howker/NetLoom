...
## Execution map after Sprint 34I environment audit

The realistic stand gate, Sprint 33A, Sprint 33B, the post-Sprint friction review, and Sprint 34A through Sprint 34H are complete. Sprint 34I performed the planned environment acceptance review without changing the repository, but the real target acceptance gate is blocked by deployment prerequisites rather than by a demonstrated product failure: no target SMTP configuration is present, and all seven discovered local NetLoom databases predate the interface-degradation outbox schema. The 34H controlled-relay acceptance and full regression remain green. One concrete operator-facing friction was exposed: pointing `delivery-status` at a pre-outbox database currently falls through to raw SQLite `no such table` output instead of a stable schema-compatibility diagnostic.
...
- [ ] Sprint 34J — stable legacy-schema diagnostics for `delivery-status`. Detect a database that predates the interface-degradation outbox before querying it; return a deterministic operator-facing schema-compatibility error and exit code instead of raw SQLite `no such table` output; preserve the command's read-only behavior and do not migrate or modify the selected database. Add regression evidence for both a pre-outbox database and a current-schema empty database. Real target SMTP acceptance remains an external deployment gate, not part of Sprint 34J.
