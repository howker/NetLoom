...
- [ ] Sprint 40 — monitoring control from the UI.
  ...
  - Target semantics: Start always uses the currently selected device's stable `DeviceId`. The monitoring panel also exposes an editable target-address field: selection pre-fills it from observed `management_address` when one exists, but the operator may enter a valid IP address when no address has yet been observed or when a session-specific override is needed. That manually entered address is launch input only and is not written back as observed `devices.management_address`. A device must exist and be selected before monitoring can start; an empty installation can therefore create a manual device first and then enter its polling address. While a schedule is running, Poll now applies to the active monitoring target rather than silently switching to another selected device.
  - Policy surface: expose the existing Engine polling knobs for the session — interval, SNMP version, port, timeout, retries, max repetitions, enabled poll kinds, and optional interface error/discard rate thresholds. These policy values and a manually entered target-address override are intentionally session-only in Sprint 40 and reset when Desktop restarts. Credentials continue to use the existing `NETLOOM_SNMP_*` environment contract; Sprint 40 does not add secret editing, a new credential store, target/profile resolution semantics, or persisted monitoring preferences.
  ...
  - Process/status semantics: show Stopped / Starting / Running / Polling / Stopping / Faulted, the active target, last successful Engine poll time, and the existing last successful topology-refresh time. The Desktop adapter owns only the Engine process it starts: redirected stdin provides cooperative `STOP`/EOF shutdown, and the Windows WPF host additionally places that child in a kill-on-close Job Object so abrupt Desktop termination cannot leave the UI-owned child behind. Independently launched Engine processes are not adopted or killed; Sprint 40 does not add global process arbitration. Child startup/runtime failure, including resource/database contention that reaches the owned process, transitions the UI state to Faulted with operator-visible diagnostics. Machine-readable stdout control/summary markers are ASCII-only, culture-invariant single lines and are flushed after emission; Desktop parses only those documented markers, never localized/free-form console text. Topology truth continues to come from the database read model.
